...
export const RequestDetailPage = ({ user }: Props) => {
  ...
  const { data: facts = [], isLoading: factsLoading } = useQuery<RequestFact[]>(
    {
      ...
      queryFn: async (): Promise<RequestFact[]> => {
        try {
          ...
          return mockRequestsFacts.filter(
            (facts: any) => facts.request_id === requestId,
          );
        }
      },
      enabled: requestId > 0,
  ...
  useEffect(() => {
    ...
    async function calculateSimilarity() {
      ...
      try {
        ...
        const currentEmbedding = await getOrCreateEmbedding(request);

        const scored = await Promise.all(
          candidateRequests.map(async (r) => {
            const embedding = await getOrCreateEmbedding(r);
            return {
              ...r,
  ...
  const canAddFact =
    user?.user_type === "employee" && request?.status !== "closed";
  const canTakeRequest =
    user?.user_type === "specialist" && request?.status === "awaiting";
  const canCloseRequest =
    user?.user_type === "specialist" &&
    request?.status === "taken" &&
    user?.id === request.moderator?.id;
  const canDeleteRequest =
    user?.user_type === "specialist" || user?.user_type === "employee";
  ...
  const handleAddFact = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    try {
      await requestsController.createFact(
        requestId,
        factTitle,
        factDescription,
        factFile,
  ...
  const handleDeleteRequest = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        navigate(
          user?.user_type === "employee" ? "/employee/requests" : "/specialist",
        );
      }, 1000);
      queryClient.removeQueries({
        queryKey: ["request", requestId],
        ...
        <Card className="p-4 mb-4">
            ...
            <Col md={6}>
              <div className="mb-3">
                ...
                <p className="text-muted">
                  {request.creator?.full_name || "Неизвестно"}
                </p>
              </div>
            </Col>
            <Col md={6}>
              <div className="mb-3">
                ...
                <p className="text-muted">
                  {request.creator?.email || "Неизвестно"}
                </p>
              </div>
            </Col>
          </Row>
          ...
          <div className="mb-3">
            ...
            <p className="text-muted">
              ...
              {request.threat_type?.categoryId &&
                ` (${request.threat_type.categoryId.name})`}
            </p>
          </div>
          ...
          <Card className="p-4 mb-4 bg-light">
            ...
            <Form onSubmit={handleAddFact}>
              ...
              <Button
                ...
                variant="primary"
                disabled={
                  addingFact || !factFile || !factTitle || !factDescription
                }
              >
        ...
        <Card className="p-4">
          ...
          {facts.length === 0 ? (
            <div className="text-muted text-center py-5">
              ...
              {canAddFact && (
                <p className="small">Добавьте первый факт выше!</p>
              )}
            </div>
          ) : (
        ...
        {user?.user_type === "specialist" && (
          <Card className="p-4 mt-4">
            ...
            {similarLoading ? (
              ...
              <p className="text-muted">Похожие заявки не найдены</p>
            ) : (
              similarRequests.map((r) => (
                <Card key={r.id}>
                  <Card.Body>
                    ...
                    <Card.Text>
                      <Badge bg="secondary">{r.threat_type?.name}</Badge>
                    </Card.Text>
